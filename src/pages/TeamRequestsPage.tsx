...
/* ── Nombre demo cuando BYPASS_AUTH está activo ──────────────── */
const MOCK_USER = 'Juan Esteban';
...
export function TeamRequestsPage() {
  ...
  // En mock usamos MOCK_USER; en producción el nombre real de la cuenta
  const nombre = config.USE_MOCK ? MOCK_USER : (account?.name ?? '');
  ...
  // En ambos modos, `todas` ya viene filtrado correctamente por el queryFn
  const allRequests = todas;
  ...
  const filtradas = filtro === 'todas'
    ? byMonth
    : byMonth.filter((r) => r.columna === filtro);
    ...
    <div style={{
      ...
      <button
        ...
        style={{
          ...
          fontSize: 12, cursor: 'pointer', padding: '4px 0', transition: 'color 0.12s',
        }}
      >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <h1 style={{
              margin: 0, fontSize: 20, fontWeight: 700, color: 'var(--txt)',
        ...
        <div style={{ position: 'relative' }}>
          <button
            ...
            style={{
              ...
              background: dropdownOpen ? c.dot + '0E' : 'var(--bg-card)',
          ...
          {dropdownOpen && (
            <div style={{
              ...
              {monthOptions.map((opt) => (
                <button
                  key={opt.value}
                  onClick={() => { setMonth(opt.value); setDropdown(false); }}
                  onMouseEnter={(e) => { if (selectedMonth !== opt.value) (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.04)'; }}
                  onMouseLeave={(e) => { if (selectedMonth !== opt.value) (e.currentTarget as HTMLElement).style.background = 'transparent'; }}
                  style={{
      ...
      <div style={{
        ...
        <TabBtn
          active={filtro === 'todas'} label="Todas"
          count={byMonth.length} color="var(--txt)"
        ...
        {tabColumnas.map((col) => {
          ...
          return (
            <TabBtn
              key={col} active={filtro === col}
              label={KANBAN_COLUMNAS[col]} count={count}
              color={COL_COLOR[col]} bg={COL_BG[col]}
      ...
      {!isLoading && filtradas.length === 0 && (
        <div style={{
          ...
          {filtro === 'todas'
            ...
            : `No hay solicitudes en "${KANBAN_COLUMNAS[filtro as KanbanColumna]}" para ${selectedLabel}.`}
        </div>
      )}
...
/* ── Tab button ──────────────────────────────────────────────── */
function TabBtn({ active, label, count, color, bg, onClick }: {
  ...
  return (
    <button
      ...
      style={{
        ...
        background: active ? (bg ?? 'var(--bg-surface)') : 'transparent',
...
function RequestRow({ request: r, onClick }: { request: Request; onClick: () => void }) {
  ...
  const isVencida = r.fechaMaxima && new Date(r.fechaMaxima) < new Date();
  ...
  return (
    <div
      ...
      style={{
        ...
        border: `1px solid ${hovered ? `${prioColor}45` : 'var(--border-subtle)'}`,
        borderRadius: 8,
        display: 'grid',
    ...
    >
      <div style={{ background: prioColor, minHeight: '100%', opacity: hovered ? 1 : 0.7, transition: 'opacity 0.15s' }} />

      <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 8, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
          <span style={{ fontFamily: 'monospace', fontSize: 10, color: hovered ? 'var(--txt-muted)' : 'var(--txt-dim)', letterSpacing: 1, transition: 'color 0.15s' }}>
        ...
        <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--txt)', margin: 0, lineHeight: 1.4 }}>
          {r.titulo}
        </p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: 0.8, textTransform: 'uppercase', color: colColor, background: colBg, border: `1px solid ${colColor}30`, borderRadius: 4, padding: '2px 8px' }}>
            {KANBAN_COLUMNAS[r.columna]}
          </span>
          <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase', color: prioColor, background: `${prioColor}15`, border: `1px solid ${prioColor}30`, borderRadius: 4, padding: '2px 8px' }}>
            {PRIORIDAD_LABEL[r.prioridad]}
          </span>
          {r.resolutor && (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--txt-muted)' }}>
              <ArrowRight size={11} />{r.resolutor}
            </span>
          )}
        </div>
      </div>
      ...
      <div style={{
        ...
        gap: 8, minWidth: 120,
        borderLeft: `1px solid ${hovered ? 'var(--border)' : 'var(--border-subtle)'}`,
        ...
        {r.fechaMaxima && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: isVencida ? 'var(--danger)' : 'var(--txt-muted)' }}>
            <Calendar size={10} />{format(new Date(r.fechaMaxima), 'd MMM', { locale: es })}
          </div>
        )}
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: 'var(--txt-muted)' }}>
          <User size={10} />
          <span style={{ maxWidth: 90, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.solicitante}</span>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 3 }}>
          {r.equipo.map((eq) => (
            <span key={eq} style={{ fontSize: 9, fontWeight: 600, letterSpacing: 0.8, textTransform: 'uppercase', color: 'var(--accent)', background: 'rgba(0,200,255,0.07)', border: '1px solid rgba(0,200,255,0.15)', borderRadius: 3, padding: '2px 7px' }}>
              {EQUIPOS[eq]}
            </span>
          ))}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, fontWeight: 600, color: 'var(--accent)', opacity: hovered ? 1 : 0, transform: hovered ? 'translateX(0)' : 'translateX(-4px)', transition: 'opacity 0.15s, transform 0.15s' }}>
    ...
    >
      <div style={{
        ...
        <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          <span style={{ fontFamily: 'monospace', fontSize: 11, color: 'var(--txt-dim)', letterSpacing: 1 }}>
            #{r.id.slice(-6).toUpperCase()}
          </span>
          <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.2, textTransform: 'uppercase', padding: '3px 10px', borderRadius: 4, color: colColor, background: colBg, border: `1px solid ${colColor}35` }}>
            {KANBAN_COLUMNAS[r.columna]}
          </span>
          <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: 0.8, textTransform: 'uppercase', padding: '3px 8px', borderRadius: 4, color: prioColor, background: `${prioColor}15`, border: `1px solid ${prioColor}30` }}>
            {PRIORIDAD_LABEL[r.prioridad]}
          </span>
          <button
            onClick={onClose}
            onMouseEnter={(e) => { const el = e.currentTarget as HTMLButtonElement; el.style.background = 'rgba(255,71,87,0.1)'; el.style.borderColor = 'rgba(255,71,87,0.3)'; el.style.color = 'var(--danger)'; }}
            onMouseLeave={(e) => { const el = e.currentTarget as HTMLButtonElement; el.style.background = 'transparent'; el.style.borderColor = 'var(--border-subtle)'; el.style.color = 'var(--txt-muted)'; }}
            style={{ marginLeft: 'auto', width: 28, height: 28, borderRadius: 6, border: '1px solid var(--border-subtle)', background: 'transparent', color: 'var(--txt-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, transition: 'all 0.12s' }}
          >
            <X size={14} />
          </button>
        </div>
        ...
        <div style={{ overflowY: 'auto', padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          <h2 style={{ fontSize: 19, fontWeight: 600, color: 'var(--txt)', lineHeight: 1.35, margin: 0 }}>
            {r.titulo}
          </h2>

          {r.descripcion && (
            <p style={{ fontSize: 13, color: 'var(--txt-muted)', lineHeight: 1.65, margin: 0, padding: '12px 14px', background: 'var(--bg-surface)', borderRadius: 7, border: '1px solid var(--border-subtle)' }}>
              {r.descripcion}
            </p>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            ...
            <MetaBlock label="Resolutor">
              {r.resolutor ? <PersonChip name={r.resolutor} color="#7c3aed" /> : <span style={{ fontSize: 13, color: 'var(--txt-muted)' }}>Sin asignar</span>}
            </MetaBlock>
            ...
            {r.fechaMaxima ? (
              <MetaBlock label="Fecha límite">
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: isVencida ? 'var(--danger)' : 'var(--warn)' }}>
                  ...
                  {format(new Date(r.fechaMaxima), "d 'de' MMMM yyyy", { locale: es })}
                  {isVencida && <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 6px', borderRadius: 3, background: 'rgba(255,71,87,0.12)', border: '1px solid rgba(255,71,87,0.25)', color: 'var(--danger)' }}>VENCIDA</span>}
                </div>
              </MetaBlock>
            ) : <div />}
            {r.categoria.length > 0 && (
              <MetaBlock label="Categorías">
          ...
          {equiposAccesibles.length > 0 && (
            <div style={{ padding: '14px 16px', background: 'var(--accent-glow)', border: '1px solid var(--accent-border)', borderRadius: 8, display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {equiposAccesibles.map((eq) => <BoardBtn key={eq} equipo={eq} onClick={() => irAlBoard(eq)} />)}
              </div>
            </div>
          )}
        </div>
...
/* ── BoardBtn ────────────────────────────────────────────────── */
function BoardBtn({ equipo, onClick }: { equipo: Equipo; onClick: () => void }) {
  const [hovered, setHovered] = useState(false);
  return (
    <button
      onClick={onClick}
      ...
      style={{
        ...
        background: hovered ? 'rgba(0,200,255,0.18)' : 'rgba(0,200,255,0.08)',
        border: `1px solid ${hovered ? 'rgba(0,200,255,0.5)' : 'rgba(0,200,255,0.25)'}`,
        color: 'var(--accent)', fontSize: 12, fontWeight: 600,
        transform: hovered ? 'translateY(-1px)' : 'translateY(0)',
        boxShadow: hovered ? '0 4px 12px rgba(0,200,255,0.15)' : 'none',
        transition: 'all 0.15s ease',
      }}
    >
      <ExternalLink size={13} />
      Board de {EQUIPOS[equipo]}
    </button>
  );
}
...
function MetaBlock({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <span style={{ display: 'block', fontSize: 9, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', color: 'var(--txt-muted)', marginBottom: 7 }}>
        {label}
      </span>
      {children}
    </div>
