...
/* ── Timer ── */
function useTimer(timerKey: string) {
  const key = `timer:${timerKey}`;
  const saved = (() => {
    try {
...
type Props = {
  equipo: Equipo;
  initialColumn: KanbanColumna;
  onClose: () => void;
  onCreate: (request: Request) => void;
};

export function CreateRequestModal({ equipo, initialColumn, onClose, onCreate }: Props) {
  ...
  const [solicitante, setSolicitante] = useState('Dev (Bypass)');
  const [resolutor, setResolutor] = useState('');
  const [prioridad, setPrioridad] = useState<Prioridad>('media');
  const [columna, setColumna] = useState<KanbanColumna>(initialColumn);

  const [equipoActual, setEquipoActual] = useState<string[]>([]);
  const [categoriaActual, setCategoriaActual] = useState<string[]>([]);
  const [sprintId, setSprintId] = useState<string | null>(null);

  const [tiempoEstimado, setTiempoEstimado] = useState('0:00');
  const [tiempoConsumido, setTiempoConsumido] = useState('0:00');

  const timer = useTimer(`create:${initialColumn}`);

  const catDefs = categorias.filter((c) => categoriaActual.includes(c.nombre));
  const sprintDef = sprints.find((s) => s.id === sprintId);
  ...
  function handleEquipo(id: string) {
    ...
    setEquipoActual(next);
  }

  function handleSprint(nextSprintId: string | null) {
  ...
  function handleCreate() {
    ...
    const newRequest: Request = {
      ...
      resolutor: resolutor.trim() || null,
      equipo: equipoActual as Equipo[],
      columna,
      prioridad,
      ...
      progreso: 0,
      ...(sprintId ? { sprintId } : {}),
      tiempoEstimado: normalizeDurationInput(tiempoEstimado) || '0:00',
      tiempoConsumido: normalizeDurationInput(tiempoConsumido) || '0:00',
    } as Request;
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
          ...
          <span
            style={{
              ...
              padding: '3px 10px',
              borderRadius: 4,
              color: COL_COLOR[columna],
              background: `${COL_COLOR[columna]}15`,
              border: `1px solid ${COL_COLOR[columna]}35`,
            }}
          >
            {KANBAN_COLUMNAS[columna]}
          </span>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 24, borderRight: '1px solid var(--border-subtle)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <FieldBlock label="Resolutor">
                <input
                  value={resolutor}
                  onChange={(e) => setResolutor(e.target.value)}
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          borderRadius: 5,
                          color: PRI_COLOR[p],
                          background: active ? `${PRI_COLOR[p]}15` : 'transparent',
                          border: `1px solid ${active ? `${PRI_COLOR[p]}35` : 'var(--border-subtle)'}`,
                          cursor: 'pointer',
                        }}
                      >
                        {PRIORIDADES[p]}
              ...
              <FieldBlock label="Puntaje">
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: 20, fontWeight: 700, fontFamily: 'var(--font-display)', color: PRI_COLOR[prioridad] }}>
                    {PUNTAJE[prioridad]}
                  </span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <FieldBlock label="Fecha de cierre">
            ...
            <FieldBlock label="Mover a">
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
                {(Object.entries(KANBAN_COLUMNAS) as [KanbanColumna, string][]).map(([col, label]) => {
                  const active = columna === col;
                  return (
                    <button
                      key={col}
                      type="button"
                      onClick={() => setColumna(col)}
                      style={{
                        padding: '6px 12px',
                        borderRadius: 6,
                        fontSize: 10,
