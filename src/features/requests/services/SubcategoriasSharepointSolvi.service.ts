...
export type SubcategoriaSP = {
  Id?: string;
  Title: string;
  Id_Categoria: string;
};

export class SubcategoriasSPService {
  ...
  private toModel(item: any): SubcategoriaSP {
    const f = item?.fields ?? {};
    return {
      Id: String(item?.id ?? ''),
      Title: f.Title,
  ...
  async getAll(opts?: GetAllOpts): Promise<SubcategoriaSP[]> {
  ...
  async getByCategoria(categoriaId: string): Promise<SubcategoriaSP[]> {
    return this.getAll({ filter: `fields/Id_Categoria eq '${this.esc(categoriaId)}'` });
  }
}
