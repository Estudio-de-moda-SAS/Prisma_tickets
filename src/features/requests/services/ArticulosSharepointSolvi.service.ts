...
export type ArticuloSP = {
  Id?: string;
  Title: string;
  Id_Subcategoria: string;
};

export class ArticulosSPService {
  ...
  private toModel(item: any): ArticuloSP {
    const f = item?.fields ?? {};
    return {
      Id: String(item?.id ?? ''),
      Title: f.Title,
  ...
  async getAll(opts?: GetAllOpts): Promise<ArticuloSP[]> {
  ...
  async getBySubcategoria(subcategoriaId: string): Promise<ArticuloSP[]> {
    return this.getAll({ filter: `fields/Id_Subcategoria eq '${this.esc(subcategoriaId)}'` });
  }
}
